...
uint64_t getLocationTypeValue(
  ...
  if (locationType != moxygen::LocationType::LargestGroup) {
    return to_underlying(locationType);
  }
...
bool isValidGroupOrderParam(uint64_t value) {
  switch (value) {
    case to_underlying(moxygen::GroupOrder::OldestFirst):
    case to_underlying(moxygen::GroupOrder::NewestFirst):
      return true;
    default:
      return false;
  }
...
bool isIntParamValid(uint64_t version, uint64_t key, uint64_t value) {
  if (moxygen::getDraftMajorVersion(version) >= 15) {
    switch (key) {
      case to_underlying(moxygen::TrackRequestParamKey::GROUP_ORDER):
      ...
      case to_underlying(moxygen::TrackRequestParamKey::FORWARD):
...
Expected<uint64_t, moxygen::ErrorCode> decodeDelta(
    uint64_t previous,
    uint64_t delta) {
  if (delta > std::numeric_limits<uint64_t>::max() - previous) {
    return makeUnexpected(moxygen::ErrorCode::PROTOCOL_VIOLATION);
  }
...
compat::Expected<compat::Unit, ErrorCode> parseParams(
...
compat::Expected<std::optional<Parameter>, ErrorCode> parseVariableParam(
    moxygen::Cursor& cursor,
    size_t& length,
    uint64_t version,
    uint64_t key,
    MoQTokenCache& tokenCache,
    ParamsType paramsType);
compat::Expected<AbsoluteLocation, ErrorCode> parseAbsoluteLocation(
    moxygen::Cursor& cursor,
    size_t& length) noexcept;
compat::Expected<SubscriptionFilter, ErrorCode> parseSubscriptionFilter(
    moxygen::Cursor& cursor,
    size_t& length) noexcept;
compat::Expected<std::optional<Parameter>, ErrorCode> parseIntParam(
    moxygen::Cursor& cursor,
    size_t& length,
    uint64_t version,
    uint64_t key);
...
void writeTrackNamespace(
    ...
    bool& error);
uint16_t*
writeFrameHeader(moxygen::BufQueue& writeBuf, FrameType frameType, bool& error);
...
compat::Expected<SubscriptionFilter, ErrorCode> parseSubscriptionFilter(
  ...
  switch (filterType->first) {
    ...
    case to_underlying(LocationType::LargestObject):
    case to_underlying(LocationType::AbsoluteStart):
    case to_underlying(LocationType::AbsoluteRange):
...
compat::Expected<std::optional<Parameter>, ErrorCode> parseVariableParam(
  ...
  p.key = key;
  const auto authKey =
      to_underlying(TrackRequestParamKey::AUTHORIZATION_TOKEN);
  const auto subscriptionFilterKey =
      to_underlying(TrackRequestParamKey::SUBSCRIPTION_FILTER);
  if (key == authKey) {
    ...
    if (!tokenRes) {
      return compat::makeUnexpected(tokenRes.error());
    }
    length -= res->first;
...
compat::Expected<compat::Unit, ErrorCode> parseParams(
  ...
  for (auto i = 0u; i < numParams; i++) {
    ...
    if (getDraftMajorVersion(version) >= 16) {
      ...
      if (decoded.hasError()) {
        return compat::makeUnexpected(decoded.error());
      }
      key = decoded.value();
      previousKey = key;
    } else {
    ...
    compat::Expected<std::optional<Parameter>, ErrorCode> res;

    if ((paramsType == ParamsType::Request &&
         key == to_underlying(TrackRequestParamKey::DELIVERY_TIMEOUT)) ||
        ((key & 0x01) == 0 &&
...
compat::Expected<ClientSetup, ErrorCode> MoQFrameParser::parseClientSetup(
    moxygen::Cursor& cursor,
    size_t length) noexcept {
...
compat::Expected<ServerSetup, ErrorCode> MoQFrameParser::parseServerSetup(
    moxygen::Cursor& cursor,
    size_t length) noexcept {
...
compat::Expected<MoQFrameParser::ParseResultAndLength<RequestID>, ErrorCode>
MoQFrameParser::parseFetchHeader(moxygen::Cursor& cursor, size_t length)
    const noexcept {
...
bool datagramTypeHasExtensions(uint64_t version, DatagramType datagramType) {
  return (to_underlying(datagramType) & 0x1);
}

bool datagramTypeIsStatus(uint64_t version, DatagramType datagramType) {
  return (to_underlying(datagramType) & 0x20);
}
...
bool datagramObjectIdZero(uint64_t version, DatagramType datagramType) {
  ...
  return (to_underlying(datagramType) & DG_OBJECT_ID_ZERO);
}
...
bool datagramPriorityPresent(uint64_t version, DatagramType datagramType) {
  ...
  return !(to_underlying(datagramType) & DG_PRIORITY_NOT_PRESENT);
}

compat::Expected<DatagramObjectHeader, ErrorCode>
...
MoQFrameParser::parseSubgroupHeader(
  ...
  if (options.priorityPresent) {
    ...
    // Leave priority as std::nullopt if not present
    CHECK_GE(getDraftMajorVersion(*version_), 15u);
    objectHeader.priority = std::nullopt;
  }
  if (parseObjectID) {
...
compat::Expected<ObjectHeader, ErrorCode>
...
MoQFrameParser::parseFetchObjectDraft15(
  ...
  if (flags &
      to_underlying(FetchHeaderSerializationBits::GROUP_ID_BITMASK)) {
  ...
  uint8_t subgroupMode = flags &
      to_underlying(FetchHeaderSerializationBits::SUBGROUP_MODE_BITMASK);
  switch (subgroupMode) {
    case to_underlying(FetchHeaderSerializationBits::SUBGROUP_ID_ZERO):
      // Subgroup ID is zero
      objectHeader.subgroup = 0;
      break;
    case to_underlying(
  ...
  if (flags &
      to_underlying(FetchHeaderSerializationBits::OBJECT_ID_BITMASK)) {
  ...
  if (flags &
      to_underlying(FetchHeaderSerializationBits::EXTENSIONS_BITMASK)) {
...
compat::Expected<MoQFrameParser::ParseResultAndLength<ObjectHeader>, ErrorCode>
MoQFrameParser::parseFetchObjectHeader(
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    if (!draft15Header) {
      return compat::makeUnexpected(draft15Header.error());
    }
    ...
    if (!objectHeader) {
      return compat::makeUnexpected(objectHeader.error());
    }
...
compat::Expected<compat::Unit, ErrorCode> MoQFrameParser::parseTrackRequestParams(
    moxygen::Cursor& cursor,
    size_t& length,
    size_t numParams,
...
compat::Expected<SubscribeRequest, ErrorCode>
MoQFrameParser::parseSubscribeRequest(moxygen::Cursor& cursor, size_t length)
    const noexcept {
  CHECK(version_.has_value())
...
compat::Expected<SubscribeOk, ErrorCode> MoQFrameParser::parseSubscribeOk(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
...
compat::Expected<Unsubscribe, ErrorCode> MoQFrameParser::parseUnsubscribe(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
...
compat::Expected<PublishRequest, ErrorCode> MoQFrameParser::parsePublish(
  ...
  if (!paramRes) {
    return compat::makeUnexpected(paramRes.error());
  }
  if (length > 0) {
    return compat::makeUnexpected(ErrorCode::PROTOCOL_VIOLATION);
  }
...
compat::Expected<PublishOk, ErrorCode> MoQFrameParser::parsePublishOk(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  CHECK(version_.has_value())
...
compat::Expected<PublishNamespace, ErrorCode>
MoQFrameParser::parsePublishNamespace(moxygen::Cursor& cursor, size_t length)
    const noexcept {
...
compat::Expected<PublishNamespaceOk, ErrorCode>
...
compat::Expected<PublishNamespaceOk, ErrorCode> MoQFrameParser::parseRequestOk(
    moxygen::Cursor& cursor,
    size_t length,
...
compat::Expected<PublishNamespaceDone, ErrorCode>
...
compat::Expected<TrackStatus, ErrorCode> MoQFrameParser::parseTrackStatus(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  CHECK(version_.has_value())
  ...
  if (!parseParamsResult) {
    return compat::makeUnexpected(parseParamsResult.error());
  }
  if (length > 0) {
    return compat::makeUnexpected(ErrorCode::PROTOCOL_VIOLATION);
  }
...
compat::Expected<TrackStatusOk, ErrorCode> MoQFrameParser::parseTrackStatusOk(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  CHECK(version_.has_value())
...
compat::Expected<TrackStatusError, ErrorCode>
MoQFrameParser::parseTrackStatusError(moxygen::Cursor& cursor, size_t length)
    const noexcept {
  return parseRequestError(cursor, length, FrameType::TRACK_STATUS_ERROR);
}

compat::Expected<Goaway, ErrorCode> MoQFrameParser::parseGoaway(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  Goaway goaway;
  auto res = parseFixedString(cursor, length);
...
compat::Expected<MaxRequestID, ErrorCode> MoQFrameParser::parseMaxRequestID(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
...
compat::Expected<RequestsBlocked, ErrorCode>
MoQFrameParser::parseRequestsBlocked(moxygen::Cursor& cursor, size_t length)
    const noexcept {
...
compat::Expected<Fetch, ErrorCode> MoQFrameParser::parseFetch(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  Fetch fetch;
  auto res = quic::follyutils::decodeQuicInteger(cursor, length);
  ...
  if (fetchTypeEnum == FetchType::STANDALONE) {
    ...
    if (!ftn) {
      return compat::makeUnexpected(ftn.error());
    }
    ...
    if (!end) {
      return compat::makeUnexpected(end.error());
    }
  ...
  if (!res5) {
    return compat::makeUnexpected(res5.error());
  }
  if (length > 0) {
    return compat::makeUnexpected(ErrorCode::PROTOCOL_VIOLATION);
  }
...
compat::Expected<FetchCancel, ErrorCode> MoQFrameParser::parseFetchCancel(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
...
compat::Expected<FetchOk, ErrorCode> MoQFrameParser::parseFetchOk(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  FetchOk fetchOk;
  auto res = quic::follyutils::decodeQuicInteger(cursor, length);
  ...
  if (!res3) {
    return compat::makeUnexpected(res3.error());
  }
  if (length > 0) {
    return compat::makeUnexpected(ErrorCode::PROTOCOL_VIOLATION);
  }
...
compat::Expected<SubscribeNamespace, ErrorCode>
...
compat::Expected<SubscribeNamespaceOk, ErrorCode>
...
compat::Expected<RequestError, ErrorCode> MoQFrameParser::parseRequestError(
    moxygen::Cursor& cursor,
    size_t length,
    FrameType frameType) const noexcept {
  RequestError requestError;
  // CHECK that frameType is one of the allowed types for this function
  CHECK(
  ...
  if (!reasonPhrase) {
    return compat::makeUnexpected(reasonPhrase.error());
  }
...
compat::Expected<UnsubscribeNamespace, ErrorCode>
...
compat::Expected<Namespace, ErrorCode> MoQFrameParser::parseNamespace(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
...
compat::Expected<NamespaceDone, ErrorCode> MoQFrameParser::parseNamespaceDone(
    moxygen::Cursor& cursor,
    size_t length) const noexcept {
  CHECK(version_)
...
compat::Expected<FullTrackName, ErrorCode> MoQFrameParser::parseFullTrackName(
    moxygen::Cursor& cursor,
    size_t& length) const noexcept {
...
compat::Expected<compat::Unit, ErrorCode> MoQFrameParser::parseExtensions(
    moxygen::Cursor& cursor,
    size_t& length,
    ObjectHeader& objectHeader) const noexcept {
  CHECK(version_.has_value())
  ...
  if (!parseExtensionKvPairsResult.hasValue()) {
    XLOG(DBG4) << "parseExtensions: error in parseExtensionKvPairs: "
               ...
               << " id=" << objectHeader.id;
    return compat::makeUnexpected(parseExtensionKvPairsResult.error());
  }
  length -= extLen->first;
  return compat::unit;
}

compat::Expected<compat::Unit, ErrorCode> MoQFrameParser::parseExtensionKvPairs(
    moxygen::Cursor& cursor,
    ObjectHeader& objectHeader,
...
compat::Expected<compat::Unit, ErrorCode> MoQFrameParser::parseExtension(
    moxygen::Cursor& cursor,
    size_t& length,
    ObjectHeader& objectHeader,
    bool allowImmutable) const noexcept {
  ...
  if (ext.type & 0x1) {
    ...
    if (isImmutableContainer) {
      moxygen::Cursor innerCursor = cursor;
      ...
      if (parseInnerResult.hasError()) {
        XLOG(DBG4)
            ...
            << " ext.type=" << ext.type << " length=" << length
            << " extLen=" << extLen->first;
        return compat::makeUnexpected(parseInnerResult.error());
      }
...
compat::Expected<std::vector<std::string>, ErrorCode>
MoQFrameParser::parseFixedTuple(moxygen::Cursor& cursor, size_t& length)
    const noexcept {
...
RequestOk RequestOk::fromTrackStatusOk(const TrackStatusOk& trackStatusOk) {
  ...
  requestOk.requestSpecificParams.emplace_back(
      to_underlying(TrackRequestParamKey::EXPIRES),
      static_cast<uint64_t>(trackStatusOk.expires.count()));

  // Add group order parameter
  requestOk.requestSpecificParams.emplace_back(
      to_underlying(TrackRequestParamKey::GROUP_ORDER),
      to_underlying(trackStatusOk.groupOrder));

  // Add the LARGEST_OBJECT param if present
  if (trackStatusOk.largest) {
    requestOk.requestSpecificParams.emplace_back(
        to_underlying(TrackRequestParamKey::LARGEST_OBJECT),
...
uint16_t* writeFrameHeader(
    ...
    bool& error) {
  size_t size = 0;
  writeVarint(writeBuf, to_underlying(frameType), size, error);
...
void writeSize(
  ...
  uint16_t sizeVal = static_cast<uint16_t>(
      ((size & 0xFF) << 8) | ((size >> 8) & 0xFF));
#endif
...
std::string MoQFrameWriter::encodeUseAlias(uint64_t alias) const {
  moxygen::BufQueue writeBuf{moxygen::BufQueue::cacheChainLength()};
  size_t size = 0;
  bool error = false;
  writeVarint(
      writeBuf, to_underlying(AliasType::USE_ALIAS), size, error);
  writeVarint(writeBuf, alias, size, error);
  CHECK(!error) << "Alias too large";
...
std::string MoQFrameWriter::encodeDeleteTokenAlias(uint64_t alias) const {
  ...
  bool error = false;
  writeVarint(
      writeBuf, to_underlying(AliasType::DELETE_ALIAS), size, error);
  writeVarint(writeBuf, alias, size, error);
  CHECK(!error) << "Alias too large";
  return writeBuf.move()->moveToFbString().toStdString();
}
...
std::string MoQFrameWriter::encodeRegisterToken(
  ...
  size_t size = 0;
  bool error = false;
  writeVarint(writeBuf, to_underlying(AliasType::REGISTER), size, error);
  writeVarint(writeBuf, alias, size, error);
  writeVarint(writeBuf, tokenType, size, error);
  writeBuf.append(tokenValue);
  size += tokenValue.size();
  CHECK(!error) << "Error encoding register token";
  return writeBuf.move()->moveToFbString().toStdString();
}
...
std::string MoQFrameWriter::encodeTokenValue(
  ...
  bool error = false;
  writeVarint(
      writeBuf, to_underlying(AliasType::USE_VALUE), size, error);
  writeVarint(writeBuf, tokenType, size, error);
  writeBuf.append(tokenValue);
  size += tokenValue.size();
  CHECK(!error) << "Error encoding token value";
  return writeBuf.move()->moveToFbString().toStdString();
}
...
WriteResult writeServerSetup(
  ...
  if (getDraftMajorVersion(version) < 15) {
    CHECK(isSupportedVersion(serverSetup.selectedVersion))
...
WriteResult MoQFrameWriter::writeFetchHeader(
  ...
  bool error = false;
  writeVarint(
      writeBuf, to_underlying(StreamType::FETCH_HEADER), size, error);
  ...
  for (const auto& ext : *extensionsToWrite) {
    ...
    if (ext.isOddType()) {
      ...
      if (!ext.arrayValue.empty()) {
        ...
        writeBuf.append(
            moxygen::Buffer::copyBuffer(ext.arrayValue.data(), ext.arrayValue.size()));
...
void MoQFrameWriter::writeParamValue(
  ...
  const auto largestObjectKey =
      to_underlying(TrackRequestParamKey::LARGEST_OBJECT);
  const auto expiresKey = to_underlying(TrackRequestParamKey::EXPIRES);
...
WriteResult MoQFrameWriter::writeDatagramObject(
  ...
  if (objectHeader.status != ObjectStatus::NORMAL || !hasLength) {
    ...
    writeVarint(
        writeBuf, to_underlying(objectHeader.status), size, error);
  } else {
    writeVarint(
        writeBuf,
        to_underlying(getDatagramType(
...
void MoQFrameWriter::writeFetchObjectDraft15(
  ...
  if (objectHeader.subgroup == 0) {
    ...
    flags |=
        to_underlying(FetchHeaderSerializationBits::SUBGROUP_ID_ZERO);
  } else if (
...
WriteResult MoQFrameWriter::writeStreamObject(
  ...
  bool shouldWriteExtensions = to_underlying(streamType) & 0x1;
  if (streamType == StreamType::FETCH_HEADER &&
...
WriteResult MoQFrameWriter::writeSubscribeRequestHelper(
  ...
  if (getDraftMajorVersion(*version_) < 15) {
    ...
    uint8_t order = to_underlying(subscribeRequest.groupOrder);
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    if (subscribeRequest.priority != kDefaultPriority) {
      ...
      priorityParam.key =
          to_underlying(TrackRequestParamKey::SUBSCRIBER_PRIORITY);
    ...
    if (subscribeRequest.groupOrder != GroupOrder::Default) {
      ...
      groupOrderParam.asUint64 =
          to_underlying(subscribeRequest.groupOrder);
    ...
    if (subscribeRequest.forward == 0) {
      ...
      forwardParam.key = to_underlying(TrackRequestParamKey::FORWARD);
...
WriteResult MoQFrameWriter::writeSubscribeUpdate(
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    CHECK(update.start.has_value()) << "start is required for draft < 15";
    CHECK(update.endGroup.has_value())
...
WriteResult MoQFrameWriter::writeSubscribeOkHelper(
  ...
  if (getDraftMajorVersion(*version_) < 15) {
    ...
    auto order = to_underlying(subscribeOk.groupOrder);
    writeBuf.append(&order, 1);
    size += 1;
  }
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    if (subscribeOk.expires.count() != 0) {
      ...
      expiresParam.key = to_underlying(TrackRequestParamKey::EXPIRES);
    ...
    if (subscribeOk.groupOrder != GroupOrder::Default) {
      ...
      groupOrderParam.key =
          to_underlying(TrackRequestParamKey::GROUP_ORDER);
      groupOrderParam.asUint64 = to_underlying(subscribeOk.groupOrder);
      requestSpecificParams.push_back(groupOrderParam);
    }
  }
  writeTrackRequestParams(
...
WriteResult MoQFrameWriter::writePublish(
  ...
  if (getDraftMajorVersion(*version_) < 15) {
    uint8_t order = to_underlying(publish.groupOrder);
    writeBuf.append(&order, 1);
    size += 1;
  }
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    if (publish.groupOrder != GroupOrder::Default) {
      ...
      groupOrderParam.key =
          to_underlying(TrackRequestParamKey::GROUP_ORDER);
      groupOrderParam.asUint64 = to_underlying(publish.groupOrder);
      requestSpecificParams.push_back(groupOrderParam);
    }
...
WriteResult MoQFrameWriter::writePublishOk(
  ...
  if (getDraftMajorVersion(*version_) < 15) {
    ...
    uint8_t order = to_underlying(publishOk.groupOrder);
    writeBuf.append(&order, 1);
    size += 1;
  }
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    if (publishOk.groupOrder != GroupOrder::Default) {
      ...
      groupOrderParam.key =
          to_underlying(TrackRequestParamKey::GROUP_ORDER);
      groupOrderParam.asUint64 = to_underlying(publishOk.groupOrder);
      requestSpecificParams.push_back(groupOrderParam);
    }
...
WriteResult MoQFrameWriter::writePublishNamespaceCancel(
  ...
  writeVarint(
      writeBuf,
      to_underlying(publishNamespaceCancel.errorCode),
      size,
      error);
...
WriteResult MoQFrameWriter::writeTrackStatusOk(
  ...
  if (getDraftMajorVersion(*version_) >= 14) {
    ...
    writeVarint(
        writeBuf, to_underlying(trackStatusOk.statusCode), size, error);
...
WriteResult MoQFrameWriter::writeSubscribeNamespace(
  ...
  if (getDraftMajorVersion(*version_) >= 16) {
    writeVarint(
        writeBuf,
        to_underlying(subscribeNamespace.options),
        size,
        error);
  }
...
WriteResult MoQFrameWriter::writeFetch(
  ...
  if (getDraftMajorVersion(*version_) < 15) {
    ...
    auto order = to_underlying(fetch.groupOrder);
    writeBuf.append(&order, 1);
    size += 1;
  }
  ...
  if (standalone) {
    writeVarint(
        writeBuf, to_underlying(FetchType::STANDALONE), size, error);
    ...
    writeVarint(
        writeBuf, to_underlying(joining->fetchType), size, error);
  ...
  if (getDraftMajorVersion(*version_) >= 15) {
    ...
    if (fetch.groupOrder != GroupOrder::Default) {
      ...
      groupOrderParam.key =
          to_underlying(TrackRequestParamKey::GROUP_ORDER);
      groupOrderParam.asUint64 = to_underlying(fetch.groupOrder);
      requestSpecificParams.push_back(groupOrderParam);
    }
  }
  writeTrackRequestParams(
...
WriteResult MoQFrameWriter::writeFetchOk(
  ...
  auto order = to_underlying(fetchOk.groupOrder);
  writeBuf.append(&order, 1);
  size += 1;
...
bool isValidDatagramType(uint64_t version, uint64_t datagramType) {
  ...
  if (majorVersion < 15) {
    ...
    return (
        datagramType <= to_underlying(
                            DatagramType::OBJECT_DATAGRAM_EXT_EOG_ID_ZERO) ||
        (datagramType >=
             to_underlying(DatagramType::OBJECT_DATAGRAM_STATUS) &&
         datagramType <=
             to_underlying(DatagramType::OBJECT_DATAGRAM_STATUS_EXT)));
  } else {
