...
void addTx(TxTotals& t, const moxygen::MoQTestPublisher::SendStats& s) {
  ...
  t.behind += s.behind.load(std::memory_order_relaxed);
}
...
void writePromFile(
  ...
  for (const auto& [kind, tx] :
    ...
    w.counter(
        ...
        tx->bytes);
    w.counter(
        fmt::format("moqmeeting_tx_{}_behind_total", kind),
        fmt::format("{} sends made while behind schedule", kind),
        tx->behind);
  }
...
folly::coro::Task<void> aggregateStats(
  ...
  while (true) {
    ...
    // Aggregating and writing metrics stretch each interval past the sleep.
    double secs = std::chrono::duration<double>(now - lastSample).count();
    lastSample = now;
    auto perSec = [secs](uint64_t delta) {
      return fmt::format("{:.0f}", delta / secs);
    };
    ...
    XLOG(INFO) << "[AGGREGATE] [" << elapsed
               ...
               << " | RxTracks: " << r.rxTracks << "/"
               << r.activeMeetings * tracksPerMeeting << " | Video obj/s rx: "
               << perSec(t.videoObjects - last.videoObjects)
               << " tx: " << perSec(t.txVideo.objects - last.txVideo.objects)
               << " | Audio dg/s rx: "
               << perSec(t.audioDatagrams - last.audioDatagrams)
               << " tx: " << perSec(t.txAudio.objects - last.txAudio.objects)
               << " | Mbps rx: " << fmt::format("{:.2f}", rxMbps)
               << " tx: " << fmt::format("{:.2f}", txMbps)
               << " | Tx behind/s video: "
               << perSec(t.txVideo.behind - last.txVideo.behind)
               << ", audio: " << perSec(t.txAudio.behind - last.txAudio.behind)
               << " | Latency(interval min/avg/max) video: "
               << formatInterval(video)
               << " ms, audio: " << formatInterval(audio) << " ms"
               << " | Resets: " << perSec(t.resets - last.resets) << "/s, "
               << t.resets << " total"
...
// The random part keeps runs apart when containers share a pid.
std::string defaultMeetingPrefix() {
  return fmt::format(
      "mtg{}x{}x{:08x}",
      getpid(),
      time(nullptr) % 1000000,
      folly::Random::rand32());
}
...
int main(int argc, char** argv) {
    ...
    auto statsDone =
        folly::coro::co_withExecutor(
            ...
            folly::coro::co_withCancellation(
                ...
                aggregateStats(client, workers, FLAGS_metrics_out, promLabels)))
            .start();
  ...
  auto formatTx = [](const TxTotals& tx) {
    return fmt::format(
        "{} objects, {} bytes, {} behind", tx.objects, tx.bytes, tx.behind);
  };
  ...
  // The file outlives the run, so its current-throughput gauges read zero.
  if (!FLAGS_metrics_out.empty()) {
    writePromFile(FLAGS_metrics_out, promLabels, r, t, 0.0, 0.0, workers);
  }
  return 0;
}
