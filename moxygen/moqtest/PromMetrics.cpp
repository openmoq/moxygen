...
void PromWriter::histogram(
  ...
  for (const auto& [extra, hist] : series) {
    auto labels = labels_.empty() || extra.empty() ? labels_ + extra
                                                   : labels_ + "," + extra;
    auto bucketPrefix = labels.empty() ? std::string() : labels + ",";
    ...
    for (size_t i = 0; i < LatencyHistogram::kNumBounds; ++i) {
      ...
      os_ << name << "_bucket{" << bucketPrefix << "le=\"" << leSec << "\"} "
          << cum[i] << "\n";
    }
    os_ << name << "_bucket{" << bucketPrefix << "le=\"+Inf\"} "
