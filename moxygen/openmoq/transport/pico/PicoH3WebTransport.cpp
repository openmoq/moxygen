...
void PicoH3WebTransport::onStreamData(
  ...
  if (it != pendingStreamNotifications_.end()) {
    ...
    if (isBidi) {
      auto bidiHandle = streamManager_->getBidiHandle(streamId);
...
void PicoH3WebTransport::processEgressEvents() {
  ...
  // Process writable streams from priority queue
  if (!priorityQueue_.empty()) {
    ...
    if (nextId.isStreamID()) {
      ...
      if (it != streamContexts_.end()) {
        // Dequeue and send data for this stream
        auto* handle = streamManager_->getOrCreateEgressHandle(streamId);
        if (handle) {
          auto streamData = streamManager_->dequeue(*handle, 16384);
...
PicoH3WebTransport::writeStreamData(
  ...
  if (result.hasValue()) {
    // Mark stream as active so processEgressEvents sends the data
    processEgressEvents();
  }
