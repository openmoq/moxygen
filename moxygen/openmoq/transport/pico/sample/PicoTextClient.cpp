...
DEFINE_string(cert_root, "", "CA bundle path for TLS (empty = skip verification)");
 ...
 public:
  PicoTextClient(
      ...
      : MoQPicoQuicEventBaseClient(
            std::move(endpoint), evb, executor, std::move(certRoot)),
        evb_(evb) {}
  ...
  void onSession(std::shared_ptr<MoQSession> session) override {
    ...
    folly::coro::co_withExecutor(
        evb_, subscribe(std::move(session), std::move(receiver), std::move(ftn)))
        .start();
  }
...
int main(int argc, char* argv[]) {
  ...
  folly::EventBase evb;
  MoQFollyExecutorImpl executor(&evb);

  PicoTextClient client(
      /*endpoint=*/"", &evb, &executor, FLAGS_cert_root);
  gClient = &client;
  ...
  folly::SocketAddress addr(url.getHost(), url.getPort(), /*allowNameLookup=*/true);
