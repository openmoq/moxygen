...
class PicoEventBaseRelayServer : public MoQPicoQuicEventBaseServer {
 public:
  PicoEventBaseRelayServer(
      ...
      : MoQPicoQuicEventBaseServer(cert, key, endpoint, evb, executor, versions),
...
int main(int argc, char* argv[]) {
  ...
  // Declare relay and server before evb/executor so they are destroyed after
  // evb. evb's destructor drains pending coroutine continuations (e.g.
  ...
  server = std::make_shared<PicoEventBaseRelayServer>(
      FLAGS_cert, FLAGS_key, FLAGS_endpoint, &evb, &executor, FLAGS_versions, relay);
