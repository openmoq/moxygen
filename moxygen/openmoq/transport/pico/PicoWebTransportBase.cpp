...
folly::Expected<folly::SemiFuture<PicoWebTransportBase::StreamData>, PicoWebTransportBase::ErrorCode>
...
void PicoWebTransportBase::onJitProvideData(
  ...
  XLOG(DBG6) << "onJitProvideData: stream=" << streamId << " dequeued=" << dataLen
             << " fin=" << fin << " isStillActive=" << isStillActive;
...
void PicoWebTransportBase::onStreamDataCommon(
    ...
    size_t length,
    bool fin) {
  XLOG(DBG5) << "onStreamDataCommon: stream=" << streamId << " length=" << length
             << " fin=" << fin;
...
void PicoWebTransportBase::onStreamResetCommon(
    uint64_t streamId,
    uint64_t errorCode) {
  XLOG(DBG2) << "onStreamResetCommon: stream=" << streamId << " error=" << errorCode;
...
void PicoWebTransportBase::onStopSendingCommon(
    uint64_t streamId,
    uint64_t errorCode) {
  XLOG(DBG2) << "onStopSendingCommon: stream=" << streamId << " error=" << errorCode;
