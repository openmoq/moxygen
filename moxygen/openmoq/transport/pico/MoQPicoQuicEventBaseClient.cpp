...
void MoQPicoQuicEventBaseClient::connect(const folly::SocketAddress& addr,
  ...
  impl_->handler =
      std::make_unique<PicoQuicSocketHandler>(evb_, impl_->quic);
...
void MoQPicoQuicEventBaseClient::onConnectionReady(void* vcnx) {
  ...
  auto sharedExecutor = std::shared_ptr<MoQExecutor>(
      static_cast<MoQExecutor*>(executor_), [](MoQExecutor*) {});
  auto moqSession = createSession(webTransport, sharedExecutor);
