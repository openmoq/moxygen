...
class FileMLogger : public MLogger {
 ...
 private:
  /**
   * Derives the output file path based on configured directory and connection
   * IDs. Returns path in order of preference: {dir}/{dcid}.mlog,
   * {dir}/{srcCid}.mlog, explicit path_, or {dir}/unknown.mlog as fallback.
   */
  std::string derivePath() const;
