...
class MoQTextClient : public Subscriber,
 ...
 public:
  ...
  std::shared_ptr<PublishNamespaceHandle> acceptPublishNamespace(
      const TrackNamespace& ns, RequestID requestID) {
    XLOG(INFO) << "acceptPublishNamespace ns=" << ns;
    auto handle = std::make_shared<PublishNamespaceHandle>(
        PublishNamespaceOk{.requestID = requestID, .requestSpecificParams = {}});
  ...
  folly::coro::Task<MoQSession::SubscribeNamespaceResult> subscribeNamespace(
    ...
    // Bridge NAMESPACE messages (draft 16+ bidi stream) to acceptPublishNamespace
    // so the relay will PUBLISH data to us for each announced namespace.
