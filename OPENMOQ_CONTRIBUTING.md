...
This repository is the [OpenMOQ](https://github.com/openmoq) fork of
[facebookexperimental/moxygen](https://github.com/facebookexperimental/moxygen).
It carries our standalone build system, CI/CD, deployment tooling, and
integration fixes on top of upstream moxygen. Contributions are welcome
from anyone.

This document sits **alongside** [CONTRIBUTING.md](CONTRIBUTING.md) —
they are complementary:

- **[CONTRIBUTING.md](CONTRIBUTING.md)** — Meta's process for contributing
  to upstream moxygen (CLA, internal-repo import, etc.). Read this when
  you want your change in the upstream project.
- **This document** — OpenMOQ's process for contributing to *this fork*.
  Read this for PRs that target `openmoq/moxygen:main` directly.

Most contributors will use both: most changes should go upstream, some
are fork-only.
...
Feel free to use any descriptive branch name. The repo is configured to
automatically delete merged branches, so there is no need to clean up.
...
## Where should my change live?

**If at all possible, submit your change to upstream
[facebookexperimental/moxygen](https://github.com/facebookexperimental/moxygen).**
Our daily upstream-sync workflow pulls merged upstream changes into this
fork automatically, so anything that lands there appears here shortly
after without any extra step from you.

Only open a PR in *this* repo if the change is genuinely OpenMOQ-specific
and has no upstream home — for example:

- Our standalone build system (`standalone/`, `openmoq/`)
- Our CI workflows (`.github/workflows/omoq-*`)
- Our deployment configs (`docker/`)
- Our integration scripts
- Docs about our fork's operation

Every fork-local patch of upstream-relevant code is a future sync
conflict. Upstreaming keeps this fork thin and maintainable. If you're
unsure which bucket your change falls into, open an issue to ask.
